...
CREATE TABLE IF NOT EXISTS MatchSummaries (
    ...
    --
    -- DOUBLE PRECISION, а не REAL: float4 хранит около семи значащих
    -- цифр, и записанное 0.9961 возвращалось из API как
    -- 0.9961000084877014 — хвост двоичного представления, который
    -- фронтенд показал бы пользователю (исправлено миграцией 016; здесь
    -- тип уже правильный, чтобы на чистой базе ошибки не было вовсе).
    final_radiant_wp DOUBLE PRECISION,
