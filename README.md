...
RANKS_BATCH_SIZE=50         # игроков в одном запросе STRATZ
```

Замеры первого живого прогона (2026-08-06):

* пакетный `players(steamAccountIds:)` у STRATZ **разрешён** нашему
  токену — в отличие от `matches(ids:)`. Квота считается по запросам, а
  не по игрокам, поэтому пакет и стал умолчанием: один запрос закрывает
  полсотни аккаунтов вместо одного у OpenDota;
