...
1. **Спринт 50 — Laning-модель** (вторая половина C5): обучаемая оценка лейнинга, каркас — `training/risk.py`.
2. **A9/A10**: даунвейт старого патча, Roshan/aegis/buyback/hero-фичи + Optuna (после 5000+ матчей).
3. **B4**: публичный релиз Win Probability — критерии выполнены, решение за владельцем.
4. **D5/D6**: нагрузочные тесты и security review — гейты Фазы 4.
5. **E1**: автозапуск стека на Windows (Планировщик задач + `wsl … make recover`) и ежедневный `dataset-export` как бэкап.
