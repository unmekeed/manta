# Manta — сводка для нового чата (обновлено 2026-07-23, после спринта 59)
...
коммитах по спринтам (1–59). Спецификация — `docs/specification/`,
...
- **Трек F — усиление модели** (docs/ML-PLAN.md, спринт 59): 8 спринтов
  F1–F8 от «сохранять сырой JSON в MinIO» до hero-эмбеддингов. Ключевые
  факты аудита: (1) JSON матча скачивается ЦЕЛИКОМ, но используется ~10%
  полей — драфт/предметы/варды/руны/Рошан/бэйбеки уже оплачены квотой и
  выбрасываются; (2) C++ ядро парсит BUYBACK/GOLD/XP, но в CH не пишет;
  (3) вся зона роста — early (Brier 0.186 vs 0.077 mid), лечится
  draft prior каскадом (F3), а не новыми разностями. Ни один спринт F
  не добавляет API-вызовов.
