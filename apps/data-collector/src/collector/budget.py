...
def daily_limit(now: datetime | None = None) -> int:
    ...
    лимиту, которого к ней больше не применяли.

    ЧЕМ ЭТО ОКАЗАЛОСЬ НА ЖИВОЙ МАШИНЕ (07.09.2026). Не тем, чем казалось.
    Потолок там был поднят руками до 15000, и в бюджет сбор не упирался
    вовсе. Зато `SHARES` остались абсолютными числами с суммой 1900 — а
    от них считается резерв под чужие гарантии. Утром это давало
    источнику потолок 13350 из 15000 вместо его доли: гарантии
    превратились обратно в «кто успел», то есть ровно в то, ради
    предотвращения чего модуль и написан (простой 2026-08-06, шесть
    часов). Поднятый потолок МОЛЧА ОТКЛЮЧИЛ механизм, который защищает
    про-эталон гейта от прожорливых соседей.

    Вывод поэтому не «поднять число», а «доли обязаны следовать за
    потолком»: ниже они из него и считаются, а не задаются рядом.
...
# Вызовов на один СОБРАННЫЙ JSON-матч.
#
# ПОЧЕМУ НЕ ДВА, как казалось. Ранг матча становится известен только
# ПОСЛЕ запроса `/matches/{id}`: листинг `/parsedMatches` отдаёт голые
# идентификаторы. Значит мы платим за КАЖДОГО осмотренного кандидата, а
# проходит отбор малая их часть — вызов тратится и на отвергнутых.
#
# ЖИВОЙ ЗАМЕР 07.09.2026, семь циклов timeline-collector: 196 осмотров,
# собрано 4 матча. Выход 2%, то есть ~49 вызовов на матч, а не 2.
# Основной отсев — не ранг (4 из 28 за цикл), а РЕЖИМ ИГРЫ: 10–22 из 28
# кандидатов это Turbo и событийные режимы, которых нет в
# `STANDARD_MODES`. Фильтр верный и смягчению не подлежит — Turbo это
# другая экономика, и модель на ней училась бы неправде.
#
# Двойка, стоявшая здесь до 196a, была ДОГАДКОЙ, не сверенной с живым
# выходом, и завышала достижимую цель в двадцать пять раз.
CALLS_PER_MATCH = 49

# ЦЕЛЬ ПО ПРИТОКУ, матчей в сутки. Число не описательное, а ПРОВЕРЯЕМОЕ:
# тест ловит конфигурацию, которая до него не дотягивает.
#
# Зачем объявлять цель. Приток ограничивали три независимых числа —
# потолок квоты, доля источника и лимит матчей за цикл, — и ни одно не
# отвечало на вопрос «сколько матчей в сутки нам нужно». Связывало их
# только совпадение.
#
# Откуда 300. Из объявленного месячного потолка и ИЗМЕРЕННОГО выхода:
# 500 000 вызовов в месяц при 49 вызовах на матч — около 10 000 матчей в
# месяц, то есть ~330 в сутки. Цель не выбрана, а посчитана.
#
# ЧЕГО ЭТИМ ПУТЁМ НЕ ДОСТИЧЬ. Пятнадцати тысяч матчей в сутки: при
# нынешнем выходе это 735 000 вызовов в сутки, около $2200 в месяц.
# Упирается не в наши настройки, а в то, что мы платим за осмотр
# кандидата, которого отвергнем. Дорога туда лежит не через деньги, а
# через отбор НА СТОРОНЕ ИСТОЧНИКА — листинг, умеющий фильтровать по
# рангу и режиму до того, как мы заплатили (ради этого в проекте и
# заводился STRATZ с его GraphQL).
TARGET_MATCHES_PER_DAY = 300
