...
MATCH_CANDIDATE = {
    "teamfights": (
        "драки с таймстампами: участники, урон, смерти, золото и опыт "
        "за драку. Прямой ответ на вопрос «кто выигрывает бои», которого "
        "у модели сейчас нет вовсе — она знает только, кто богаче"),
    "draft_timings": (
...
PLAYER_CANDIDATE = {
    "lh_t": (
        "ПОМИНУТНЫЕ добитки — готовый ряд той же длины, что gold_t. "
        "Разделяет «богат, потому что фармит» и «богат, потому что "
        "убивает»: сейчас модель этих случаев не различает"),
    "dn_t": "поминутные денаи: давление на линии, которого модель не видит",
    "hero_damage_t": (
        "ПОМИНУТНЫЙ урон по героям. Сильнейший из неиспользованных "
        "рядов: показывает, кто ведёт бой, а не кто накопил"),
    "hero_healing_t": "поминутное лечение — цена размена в драках",
    "camps_stacked_t": "поминутные стаки лагерей: работа саппорта",
