...
# Коды, по которым матч не появится НИКОГДА (спринт 197).
#
# 404 — OpenDota про такой матч не знает; 410 — знал и удалил; 400 — id
# не годится вовсе. Повторять их бессмысленно, а стоит каждый повтор
# ровно столько же, сколько успешный осмотр: detail-вызов — самый дорогой
# в цикле.
#
# Чего здесь НЕТ и почему. 403 и 401 — про наш ключ, а не про матч: они
# одинаково относятся ко всем кандидатам, и запоминать по ним отдельные
# match_id значило бы выбросить пол-листинга из-за протухшего ключа.
# 5xx — чужая поломка, к утру пройдёт.
PERMANENT_DETAIL_STATUSES = {400, 404, 410}
        ...
        try:
            for mid in self._candidates(stats):
                ...
                try:
                    ...
                    # Разбивка по коду. «ошибка запроса: 12» не говорит,
                    # чинить ли что-то у себя или это чужой сервер лежит,
                    # — а решения по постоянной и временной ошибке разные.
                    key = f"  · HTTP {code}"
                    stats[key] = stats.get(key, 0) + 1
                    if code in PERMANENT_DETAIL_STATUSES:
                        # Вердикт не изменится НИКОГДА, значит матч
                        # отвергнут — на том же основании, по какому сюда
                        # попадают отсеянные фильтром.
                        #
                        # Без этой строки постоянная ошибка стоила бы
                        # detail-вызова В КАЖДОМ ЦИКЛЕ, пока матч не
                        # уползёт из окна листинга. Замер 07.09: 2–12
                        # ошибок на 28 осмотров — до 40% самого
                        # дефицитного ресурса, и платили мы за них снова
                        # и снова.
                        #
                        # Та же ошибка, что стоила 82 часов реплейному
                        # пути 2026-07-31: постоянный отказ обрабатывался
                        # как временный.
                        self._rejected.add(mid)
                    continue
                except requests.RequestException as e:
                    # Обрыв, таймаут, DNS — сторона ответила не кодом, а
                    # молчанием. Это ВРЕМЕННОЕ и в отказы не идёт:
                    # запомнить матч здесь значило бы выбросить его
                    # навсегда из-за чужой минутной сетевой неурядицы.
                    logger.warning("матч %d: %s — пропуск", mid, e)
                    stats["ошибка запроса"] += 1
                    stats["  · обрыв"] = stats.get("  · обрыв", 0) + 1
                    continue
