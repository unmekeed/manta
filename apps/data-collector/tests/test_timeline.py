...
def test_runner_inserts_and_marks(monkeypatch):
    ...
    # Спринт 185: +2 — готовых рядов OpenDota (урон и лечение по героям)
    # в фикстуре нет, и это ПРОПУСК, а не «никто не бил».
    # Спринт 187: +5 — состава в фикстуре нет (герои её игроков не
    # заданы), и это пропуск, а не «составы симметричны».
    # Ревизия 190 сняла 15 фич из 22 (драки, роли, три «фарма»), и число
    # здесь упало с 28 ровно на 15. Не сошлось — значит ревизия задела
    # что-то сверх списка.
    assert lines[0].count("nan") == 13
