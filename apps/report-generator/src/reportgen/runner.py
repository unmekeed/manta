...
WP_PASSTHROUGH_FEATURES = [
    ...
    # Волны 185-187 — И ТА ЖЕ ОШИБКА ВТОРОЙ РАЗ. Двадцать две фичи ушли в
    # обучение тремя спринтами подряд, и ни одна не была дописана сюда;
    # test_wp_features_sync снова горел от самого спринта 185, но набор
    # тестов report-generator в те спринты никто не запускал. Комментарий
    # выше был написан ровно про этот сценарий — и не помог, потому что
    # предупреждение читают те, кто и так помнит.
    #
    # Здесь семь, а не двадцать две, потому что ревизия спринта 190
    # оставила от волн только измеримо полезное; остальное удалено из
    # модели. Что осталось — обязано доезжать до Predict.
