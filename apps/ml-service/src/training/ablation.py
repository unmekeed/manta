...
GROUPS: dict[str, list[str]] = {
    ...
    **{f"G1_окно_{w // 60}мин": [rate_name(m, w) for m in RATE_METRICS]
    ...
    # Волны 185–187 ПОСЛЕ РЕВИЗИИ (спринт 190). Из девяти семейств
    # осталось два — те, по которым вердикт ещё не вынесен или вынесен
    # «полезна»:
    #
    #   S185_бой   покрытие 41%, абляция ОТКАЗАЛАСЬ судить: сигнал лишь в
    #              1299 матчах из нужных 2000. Вернуться при росте данных.
    #   S187_состав_атрибуты  Δ +0.00255 ±0.00115, на ранней фазе +0.00399.
    #              Единственное, что сработало из двадцати двух фич.
    #
    # Удалённые семейства (фарм, все три варианта драк, роли) сюда не
    # возвращаются: они удалены из модели, и измерять больше нечего.
    "S185_бой": ["hero_damage_diff", "hero_healing_diff"],
    "S187_состав_атрибуты": ["melee_diff", "attr_str_diff", "attr_agi_diff",
