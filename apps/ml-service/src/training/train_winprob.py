...
MODEL_VERSION = "0.9.0"  # 0.9.0: G1 производные (+15); draft_prior снят ревизией F
...
MONOTONE = {
    ...
    # A14. Перевес в точке боя — направление бесспорно: больше своих
    # рядом со стычкой, выше шансы. Собранность оставлена без
    # ограничения: кучность выигрывает драку, но в фарм-фазе означает
    # потерю карты, и знак зависит от момента — пусть решает модель.
    "local_manpower_diff": 1,
    "spread_diff": 0,
    ...
    "roshan_diff": 1,
    "aegis_alive": 1,
    # Готовый выкуп — чистый резерв: больше героев могут вернуться в
    # решающую драку, тем выше шансы. Направление бесспорно.
    "buyback_availability": 1,
    "first_blood": 1,
    "item_value_diff": 1,
    "key_items_diff": 1,
    "levels_diff": 1,
    ...
    "buybacks_diff": 0,
    # Золото в кармане копится и намеренно (под бэйбек, под тайминг), и
    # по ошибке (не успели закупиться) — направление вклада неочевидно.
    "unspent_gold_diff": 0,
    "obs_wards_diff": 0,
    # Площадь обзора — тот же случай, что счётчик вардов: контроль карты
    # полезен, но знак вклада зависит от фазы, и связывать модель рано.
    "vision_coverage_diff": 0,
    "sen_wards_diff": 0,
    "runes_diff": 0,
}
...
# Спринт 185: готовые ряды OpenDota — тоже БЕЗ монотонных ограничений, и
# по той же причине, что производные выше.
#
# Направление кажется бесспорным только у двух из пяти. Добитки и урон по
# героям — да, больше это лучше. А вот лечение: перевес по лечению
# означает и «мы лучше держимся», и «нас продавливают, и мы отхиливаемся»
# — знак зависит от режима. Стаки лагерей вообще про темп фарма, а не про
# преимущество: команда, стакающая на двадцатой минуте, может как готовить
# рывок, так и отставать.
#
# Ограничение — это доменное утверждение, заданное модели ЗАРАНЕЕ. Окажись
# оно неверным, абляция покажет «фича бесполезна», и мы не отличим «идея
...
def push_with_gate(artifact: dict, out_path: Path, logger_, ds=None
    ...
    if ok:
        ...
        # улучшению над якорем.
        #
        # Заводится же она сама лишь на ПЕРВОЙ ВЕРСИИ ВООБЩЕ — когда
        # выбирать не из чего. Правка спринта 209: раньше якорь вставал
        # на первой продвинутой версии при любой истории, а такой
        # версией по построению оказывается ХУДШАЯ из виденных, и планка
        # встала бы ровно там, куда съехал прод. Замер 08.09.2026: пять
        # последних версий 0.1581 → 0.1572 → 0.1599 → 0.1594 → 0.1617.
        #
        # Когда история есть, выбор якоря — решение, а не побочный
        # эффект промоушена: его делает `training.anchor`, честно сравнив
        # версии на одном сегодняшнем holdout. Пока выбор не сделан,
        # храповик молчит — и ГОВОРИТ об этом в объяснении гейта, то есть
        # и в Telegram.
        #
        ...
        if beat_champion or (prod_bytes is None and not champ_version):
