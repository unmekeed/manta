...
`RETRAIN_INTERVAL_S` (6 ч) смотрит размер витрины и переобучает, когда он
изменился на ≥ `RETRAIN_MIN_NEW_MATCHES` (20) с прошлого обучения в этом
процессе (по модулю — устойчиво к сбросу/перестройке витрины; сравнение с
production ушло бы в минус и застряло). Первый прогон при ≥
`RETRAIN_MIN_TOTAL_MATCHES` (50) обучает сразу. Продвижение решает гейт по
про-эталону, поэтому автономная деградация невозможна: худшая версия
сохраняется, но не сервится.
