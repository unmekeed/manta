...
витрины на входе, весь вектор в порядке FEATURES на выходе.
...
def full_row() -> dict:
    ...
    return {
        ...
        "levels_diff": -4,
        # Спринт 185, ревизия 190: боевые ряды OpenDota. Числа попарно
        # РАЗНЫЕ — эталон ловит перестановку колонок только тогда, когда
        # значения различаются; одинаковые значения делали бы соседние
        # фичи взаимозаменяемыми, и тест это сразу показал.
        "hero_damage_diff": 15300, "hero_healing_diff": -640,
        # Спринт 187, ревизия 190: состав команд по свойствам героев.
        # Значения уникальны В ПРЕДЕЛАХ ВСЕГО вектора и потому
        # неправдоподобно велики: настоящая разность состава лежит в
        # пределах ±5. Эталон проверяет ПРОВОДКУ и ПОРЯДОК колонок.
        "melee_diff": 11,
        "attr_str_diff": 12,
        "attr_agi_diff": 13,
        "attr_int_diff": 14,
...
def scenarios() -> list[dict]:
    ...
    for key in ("hero_damage_diff", "hero_healing_diff",
    ...
    return [
        ...
        {"name": "реплейный путь: нет рядов OpenDota",
         "why": "урон, лечение и состав приходят только с JSON-ответом "
                "(спринты 185 и 187). У матча, разобранного из демки, их "
                "нет — и это пропуск, а не «никто не бил»",
         "row": replay_row},
        ...
        {"name": "первая строка матча",
         "why": "производных ещё не существует (dt = 0), а уровни уже есть: "
                "NaN обязан прийти ровно в 15 колонок — производные, и ни\n                в одну сверх них",
         "row": first_row},
    ]
