...
PATTERNS=(
    ...
    "vite --host:frontend"
    # ДАШБОРД НЕ ОСТАНАВЛИВАЕМ (спринт 74). С панелью управления он стал
    # единственным местом, откуда стек можно поднять обратно: убив его
    # здесь, мы бы отрезали кнопку «Поднять всё» вместе с сервисами.
    # Остановить его отдельно: pkill -f scripts/dashboard.py
)
