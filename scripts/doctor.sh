    ...
    if [ -z "$vols" ]; then
        ...
        for v in $vols; do
            ...
            if [ -z "$mp" ] || [ ! -d "$mp" ]; then
                ...
                # ПУСТОЙ ТОМ ЗНАЧИТ ДВА РАЗНЫХ. Либо сервис пишет мимо
                # (дефект Kafka, спринт 193), либо сервисом просто не
                # пользуются. Различить их изнутри проверки нельзя, и
                # ровно поэтому исключение здесь ОДНО и оно УСЛОВНОЕ.
                #
                # MLflow поднят всегда, но реестр моделей живёт в
                # S3/MinIO (REGISTRY_BACKEND=s3 по умолчанию). Пока это
                # так, писать ему нечего и пустой том — норма. Переключат
                # реестр на mlflow — исключение исчезнет само, и FAIL
                # вернётся. Безусловный список исключений тем и плох, что
                # переживает причину, по которой заведён.
                if [ "$v" = "manta_mlflow_data" ] \
                   && [ "${REGISTRY_BACKEND:-s3}" != "mlflow" ]; then
                    ok "$v пуст — MLflow не используется (реестр в S3)"
                else
...
echo "== Диск (реплейный путь ограничен ИМ, а не квотой)"
#
# ЗАЧЕМ. Реплей весит ~58 МиБ, и путь, который их качает, упирается не в
# деньги, а в место. При этом до спринта 200 доктор про диск не знал
# НИЧЕГО: поднять OPENDOTA_LIMIT и получить «no space left on device»
# посреди ночи было делом одной строки в env-файле.
#
# Смотрим корень, а не каталог Docker: тома, образы, логи и база живут на
# одном разделе, и кончится он для всех сразу.
disk_avail=$(df -Pk / 2>/dev/null | awk 'NR==2 {print $4}')
disk_pct=$(df -P / 2>/dev/null | awk 'NR==2 {gsub(/%/,"",$5); print $5}')
if [ -z "$disk_avail" ]; then
    warn "df не отвечает — свободное место неизвестно"
else
    avail_gib=$((disk_avail / 1024 / 1024))
    # Порог в гигабайтах, а не в процентах: ёмкость дисков разная, а
    # суточный аппетит реплейного пути абсолютный. 10 ГиБ — это меньше
    # двух суток при 96 матчах в сутки, то есть предупреждение приходит
    # тогда, когда ещё есть время что-то сделать.
    if [ "$avail_gib" -lt 5 ]; then
        fail "на диске ${avail_gib} ГиБ (занято ${disk_pct}%) — реплеи скоро перестанут скачиваться"
    elif [ "$avail_gib" -lt 10 ]; then
        warn "на диске ${avail_gib} ГиБ (занято ${disk_pct}%) — при 58 МиБ на реплей это меньше двух суток запаса"
    else
        # Сколько суток продержимся при нынешнем притоке. Считается по
        # ФАКТИЧЕСКОМУ числу собранных за сутки реплеев, а не по лимитам
        # из конфига: лимит говорит о намерении, а место кончается от
        # того, что скачано на самом деле.
        got=$(docker exec manta-postgres-1 psql -U dota -d manta -tAc \
              "SELECT count(*) FROM CollectedMatches
                WHERE has_replay AND collected_at > NOW() - INTERVAL '1 day'" \
              2>/dev/null | tr -d ' ')
        if [ -n "$got" ] && [ "$got" -gt 0 ] 2>/dev/null; then
            days=$(( avail_gib * 1024 / (got * 58) ))
            ok "на диске ${avail_gib} ГиБ (занято ${disk_pct}%); при ${got} реплеях/сут хватит на ~${days} сут"
        else
            ok "на диске ${avail_gib} ГиБ (занято ${disk_pct}%)"
        fi
    fi
fi

echo "== Страница состояния сбора (запрос ЦЕЛИКОМ, а не только маршрут)"
#
# ЗАЧЕМ ЭТО ЗДЕСЬ. Обработчик /admin/status покрыт тестами, но ЕГО SQL —
# нет: без живой базы запрос не выполнить, и в спринте 198 он уехал в
# production ни разу не запущенным. Сломался первым же вызовом — один
# параметр использовался в двух местах с разными типами, и pgx не смог
# его закодировать. Тесты при этом были зелёные и остались бы зелёными.
#
# Проверка «маршрут отвечает 401» такое не ловит: 401 приходит ДО
# обработчика, от middleware. Нужен полный проход с настоящим токеном —
# ровно правило «вход должен быть боевым».
if [ -z "${JWT_PRIVATE_KEY_FILE:-}" ] || [ ! -f "${JWT_PRIVATE_KEY_FILE:-/nonexistent}" ]; then
    warn "JWT_PRIVATE_KEY_FILE не задан — страницу состояния не проверить"
else
    st_token=$(./scripts/issue-token.sh admin 2>/dev/null || true)
    if [ -z "$st_token" ]; then
        warn "не удалось выпустить admin-токен — страницу состояния не проверить"
    else
        st_url="https://127.0.0.1:8080/api/v1/admin/status"
        st_body=$(curl -sk --max-time 10 -H "Authorization: Bearer $st_token" \
                       "$st_url" 2>/dev/null || true)
        st_code=$(curl -sk --max-time 10 -o /dev/null -w '%{http_code}' \
                       -H "Authorization: Bearer $st_token" "$st_url" \
                       2>/dev/null || echo 000)
        case "$st_code" in
            200)
                # Отвечает — но пустой список источников означает, что
                # сбора не было НИ ОДНОГО за неделю. Это не «страница
                # сломана», а «система стоит», и путать их нельзя.
                if printf '%s' "$st_body" | grep -q '"sources":\[\]'; then
                    fail "страница состояния пуста: ни один источник не собирал ничего неделю"
                else
                    ok "страница состояния отвечает и знает про источники"
                fi ;;
            404) fail "страница состояния: 404 — шлюз крутит код до спринта 198" ;;
            401|403) fail "страница состояния: $st_code при свежем admin-токене — расходятся ключи шлюза и issue-token" ;;
            000) warn "страница состояния недоступна (шлюз не отвечает на 8080)" ;;
            *)   fail "страница состояния: HTTP $st_code — $(printf '%s' "$st_body" | head -c 200)" ;;
        esac
    fi
fi
