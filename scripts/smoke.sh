...
# Отдельная форма ДЛЯ КОМАНД, ЧИТАЮЩИХ STDIN. Без `-i` docker не
# прокидывает контейнеру стандартный ввод: процесс получает EOF сразу.
# Для `kafka-topics.sh --list` это безразлично, для продюсера — смертельно
# (см. блок публикации ниже).
KAFKA_IN="docker exec -i manta-kafka-1 /opt/kafka/bin"
...
# Сумма конечных смещений топика по всем партициям.
#
# Нужна, чтобы отличить «сообщение опубликовано» от «команда публикации
# завершилась без ошибки» — а это, как выяснилось, разные утверждения.
# Пустая строка означает «измерить не удалось», и вызывающий обязан
# сказать об этом вслух, а не считать её нулём.
end_offsets() {
    $KAFKA_BIN/kafka-get-offsets.sh --bootstrap-server localhost:9092 \
        --topic "$TOPIC" 2>/dev/null |
        awk -F: 'NF >= 3 { s += $3; n += 1 } END { if (n) print s }'
}
...
#
# ПУБЛИКАЦИЯ ИЗМЕРЯЕТСЯ, А НЕ ОБЪЯВЛЯЕТСЯ. Первая редакция этого прогона
# звала продюсера через `docker exec` БЕЗ `-i`. Без него стандартный ввод
# контейнеру не отдаётся: продюсер читает EOF сразу, не публикует ничего
# и выходит с нулём. Проверка честно печатала «событие опубликовано»,
# честно ждала таймаут и честно сообщала «отчёт не появился» — при
# полностью исправном конвейере. Лаг консьюмер-группы был нулевым,
# генератор в те же минуты производил отчёты по другим матчам, и диагноз
# уводил в сторону тем вернее, чем внимательнее его читали.
#
# Это ровно то, от чего лечится проект: ОБЪЯВЛЕНИЕ — НЕ ФАКТ. Сторож,
# сам не проверяющий свой вход, сообщает о поломке того, до чего не
# дотянулся. Поэтому ниже сравниваются конечные смещения топика до и
# после: единственное наблюдаемое доказательство, что сообщение есть.
trace=$(date +%s%N)
...
off_before=$(end_offsets)

if ! printf '%s\n' "$envelope" | $KAFKA_IN/kafka-console-producer.sh \
        --bootstrap-server localhost:9092 --topic "$TOPIC" >/dev/null 2>&1; then
    fail "событие не опубликовано — Kafka недоступна"
    exit 1
fi

off_after=$(end_offsets)
if [ -z "$off_before" ] || [ -z "$off_after" ]; then
    # Не выдаём неизмеренное за измеренное: дальше идём, но читатель
    # обязан знать, что публикация НЕ ПОДТВЕРЖДЕНА, и при таймауте это
    # первое, что надо исключить.
    warn "смещения топика не измерены — публикация не подтверждена"
elif [ "$off_after" -le "$off_before" ]; then
    fail "продюсер вышел успешно, а сообщения в топике нет: конец топика" \
         "как был ($off_before). Дальше ждать нечего — упало ДО конвейера"
    echo
    echo "   Первое, что проверить: отдан ли контейнеру stdin."
    echo "   'docker exec' БЕЗ -i читает EOF сразу и публикует пустоту,"
    echo "   возвращая ноль. Проверить руками:"
    echo "      echo '$envelope' \\"
    echo "        | $KAFKA_IN/kafka-console-producer.sh \\"
    echo "            --bootstrap-server localhost:9092 --topic $TOPIC"
    exit 1
else
    ok "событие опубликовано (конец топика $off_before → $off_after)"
fi
...
#
# Сюда мы доходим, УЖЕ убедившись, что сообщение в топике есть (смещения
# сдвинулись выше). Без той проверки нулевой лаг читался бы как «событие
# прочитано и упало внутри» — и был бы враньём: у непроизведённого
# сообщения лаг тоже нулевой.
...
echo "      (публикация подтверждена сдвигом смещений выше)."
