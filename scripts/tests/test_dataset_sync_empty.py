...
def test_import_skips_empty_tables_in_loop():
    """Проверка на уровне текста скрипта: ВСЕ три петли импорта
    (Replacing, Raw и Postgres) вызывают is_empty_dump. Пропущенная петля —
    тот же обрыв.

    Постгресовая петля появилась в спринте 156, и пустой дамп ей опаснее,
    чем клику: список колонок она берёт из архива, и на пустом файле
    COPY получил бы пустой список — синтаксическую ошибку вместо
    внятного «в архиве пусто».
    """
    src = SYNC.read_text(encoding="utf-8")
    body = src[src.index("import_dataset()"):]
    assert body.count("is_empty_dump") == 3, (
        "все три петли импорта должны пропускать пустые дампы")
