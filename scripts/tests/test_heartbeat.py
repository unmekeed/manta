...
# Заглушки печатают вывод doctor В ТОЧНОСТИ так, как он выглядит на самом
# деле — С ЦВЕТОМ (спринт 193).
#
# Раньше здесь стоял чистый текст, и из-за этого проверка «строки FAIL
# доезжают до сообщения» была зелёной ТРИ МЕСЯЦА, пока в жизни не доезжала
# ни одна: doctor печатает `   \033[31mFAIL\033[0m текст`, то есть после
# отступа идёт escape-последовательность, а heartbeat искал `^\s+FAIL`.
# Совпадений не было никогда, и 🔴 приходил без единой причины.
#
# Заглушка, которая чище реальности, — это тест, подтверждающий то, чего
# не бывает. Цена конкретная: 3 сентября 2026 реплейный путь и генерация
# отчётов стояли трое суток при исправно приходящих алертах.
_RED = "\033[31m"
_GREEN = "\033[32m"
_OFF = "\033[0m"
...
def _ok(text):
    return f"    {_GREEN} OK {_OFF} {text}"


def _fail(text):
    return f"   {_RED}FAIL{_OFF} {text}"


HEALTHY = "== всё\n" + _ok("всё хорошо") + "\n>> ЗДОРОВ (warn: 0)"
BROKEN = ("== данные\n"
          + _fail("ReplayEvents: последняя вставка 124ч назад") + "\n"
          + _fail("витрина не растёт 124ч (9882 матчей)") + "\n"
          ">> ПРОБЛЕМ: 2 (warn: 0)")
