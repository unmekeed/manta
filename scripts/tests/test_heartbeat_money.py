...
def test_the_heartbeat_reads_the_doctor_instead_of_the_database():
    ...
    Проверяется СВОЙСТВО, а не имя переменной. Раньше здесь искалось
    буквальное `doctor_out`, и спринт 193 уронил тест по ложной причине:
    вывод доктора стали чистить от цвета в `plain_out`, свойство при этом
    сохранилось полностью. Имя — артефакт реализации; вопрос же в том,
    ходит ли сторож в базы сам.
    """
    block = BEAT.split("Деньги за месяц", 1)[1][:600]
    # Источник цифры — переменная, полученная из вывода доктора. Обе
    # формы законны: сырой вывод и очищенный от escape-последовательностей.
    assert any(v in block for v in ("doctor_out", "plain_out")), (
        "строка про деньги считается не из отчёта доктора")
    assert "plain_out=$(printf" in BEAT or "doctor_out=$(./scripts/doctor.sh" in BEAT
