...
def test_doctor_does_not_hang_on_an_open_stdin(tmp_path):
    ...
    `docker exec -i` тянет из стандартного ввода. Доктора запускают из
    скриптов и из тестов, где на входе висит труба, которую никто не
    закрывает, — и проверка вставала намертво. Поймалось это здесь же:
    прогон всего набора замер на `daily-report.sh`, который зовёт доктора.
