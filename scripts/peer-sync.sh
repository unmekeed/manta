...
# «Соседей нет» — это КОНФИГУРАЦИЯ, а не отказ (спринт 163).
#
# Обмен придуман для двух собирающих машин. Когда машина одна — а с
# 2026-09-01 это штатное состояние проекта, — обменивать не с кем, и
# сказать об этом надо ровно один раз, спокойным голосом.
#
# До этого спринта отсутствие MANTA_PEER_HOSTS было `die`, то есть красным
# алертом в Telegram. В расписании обмен стоит на 04:00 ежесуточно —
# значит одиночная машина слала бы «ОБМЕН НЕ УДАЛСЯ» каждую ночь, вечно.
# Ложная тревога, повторяемая по расписанию, не просто бесполезна: она
# обучает не читать канал, и следом мимо проходит настоящая беда. Ровно
# так в этом проекте тринадцать дней никто не замечал, что бэкап не
# снимается.
skip() { echo "Обмен не настроен: $1"; exit 0; }

[ -n "$PEER_HOSTS" ] || skip "MANTA_PEER_HOSTS пуст — соседних машин нет"

# Ниже этой черты обмен ОБЪЯВЛЕН: соседи названы, значит слепков ждут. Всё,
# что мешает их привезти, — настоящая поломка, и молчать о ней нельзя.
# «Выключено» и «настроено, но не работает» обязаны различаться: во втором
# случае половина датасета не приезжает, а выглядит всё зелёным.
