...
TALLY_PATH = CREDENTIAL_DIR / "daily-tally.json"


def _today() -> str:
    """Дата по UTC.

    Когда именно Valve сбрасывает суточные лимиты, неизвестно, и гадать
    незачем: важно, чтобы граница была ОДНА и та же от запуска к запуску.
    UTC этим свойством обладает, местное время с переводом часов — нет.
    """
...
def read_tally() -> dict:
    """Сколько солей аккаунт отдал СЕГОДНЯ, за все запуски.

    Без этого счёта замер меряет не то. Первый прогон дал 100 солей и
    кончился списком; второй дал 47 и упёрся в молчание GC — и вердикт
    поделил 2000 на 47, будто первых ста не было. Суточный потолок это
    свойство АККАУНТА за сутки, а не одного запуска.
    """
    import json
    try:
        data = json.loads(TALLY_PATH.read_text(encoding="utf-8"))
    except Exception:  # noqa: BLE001 — файла нет или он испорчен
        return {"date": _today(), "ok": 0, "runs": 0}
    if data.get("date") != _today():
        return {"date": _today(), "ok": 0, "runs": 0}
    return {"date": data["date"], "ok": int(data.get("ok", 0)),
            "runs": int(data.get("runs", 0))}


def add_to_tally(ok: int) -> dict:
    import json
    tally = read_tally()
    tally["ok"] += int(ok)
    tally["runs"] += 1
    try:
        ...
        TALLY_PATH.write_text(json.dumps(tally), encoding="utf-8")
    except OSError as exc:
        print(f"счёт не сохранён ({exc}) — следующий прогон его не увидит")
    return tally
...
def probe_details(match_ids: list[int], delay_s: float, limit: int) -> int:
    ...
    tally = add_to_tally(ok)
    print("\n=== ИТОГ: одиночные детали ===")
    print(f"  получено солей:   {ok}   (за этот прогон)")
    print(f"  отказов:          {failed}  {reasons or ''}")
    print(f"  соль проверена на CDN: {verified} из первых 5")
    print(f"  заняло:           {minutes:.1f} мин")
    print(f"  ВСЕГО ЗА СУТКИ:   {tally['ok']} солей "
          f"за {tally['runs']} прогон(ов), UTC-дата {tally['date']}")

    if ok == 0 and tally["ok"] == ok:
        # Экстраполировать нечего, и делать этого нельзя. Прежняя версия
        # делила 2000 на max(ok, 1) и печатала «нужно ~2000 аккаунтов» —
        # число, которое выглядит замером, а означает лишь ноль в
        # знаменателе.
        ...
        print("    молчание GC                — эндпоинт снят ЛИБО лимит;")
        ...
        # match_id дали, столько и обработали.
        print(f"\n  ПОТОЛОК НЕ ДОСТИГНУТ: отказов подряд нет, кончились "
              f"матчи ({attempted} шт).")
        print(f"  Аккаунт отдал за сутки уже {tally['ok']} и не отказал.")
        print("  Дальше: тот же прогон с бо́льшим --from-steam / --limit.")
    else:
        # Стена настоящая. Считаем от СУТОЧНОГО итога, а не от прогона:
        # первый прогон дал 100 солей и кончился списком, второй дал 47 и
        # упёрся в молчание — делить 2000 на 47, будто первых ста не
        # было, значит занизить потолок вдвое с лишним.
        print(f"\n  ВЕРДИКТ: стена после {attempted} попыток в этом прогоне.")
        print(f"  Аккаунт отдал за сутки {tally['ok']} солей.")
        print(f"  Под 2000 матчей в сутки нужно ~{-(-2000 // tally['ok'])}"
              f" аккаунтов — ЕСЛИ это суточный лимит.")
        print()
        print("  ВАЖНО: отличить суточный лимит от временного троттлинга")
        print("  одним прогоном НЕЛЬЗЯ. Молчание GC выглядит одинаково в")
        print("  обоих случаях. Решающая проверка — подождать 30 минут и")
        print("  повторить:")
        print("    пошли соли снова  => это троттлинг, суточный потолок выше;")
        print("    снова молчание    => это суточный лимит, число выше верное.")
    return 0
