...
client.on('loggedOn', () => {
  ...
  // Ждём необязательное событие ограниченное время, дальше печатаем
  // вердикт без него. Пяти секунд на живом аккаунте не хватило дважды,
  // поэтому пятнадцать: ответ про limited нужен для решения по GC, и
  // ради него не жалко подождать. Вход к этому моменту уже состоялся —
  // главный ответ у нас есть в любом случае.
