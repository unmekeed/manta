...
# ЦВЕТ СНИМАЕТСЯ ДО РАЗБОРА (спринт 193). doctor печатает
# `   \033[31mFAIL\033[0m текст`, то есть после отступа идёт не слово
# FAIL, а escape-последовательность. Выражение `^\s+FAIL` не совпадало
# НИКОГДА, список проблем всегда был пуст, и 🔴 три месяца приходил без
# единой причины — заголовок и счётчик собираются отдельно, поэтому
# сообщение выглядело осмысленным.
#
# Цену этого видно на 3 сентября 2026: реплейный путь и генерация отчётов
# стояли трое суток при исправно приходящем 🔴, потому что сообщение не
# называло, что именно сломалось, и владелец шёл смотреть не сразу.
#
# Тест на список проблем существовал и был зелёным: его заглушка печатала
# FAIL БЕЗ цвета. Заглушка оказалась чище реальности, и проверка
# подтверждала то, чего в жизни не бывает.
...
money=$(printf '%s\n' "$plain_out" | grep -oE 'OpenDota за месяц: [^(]+\([0-9]+% потолка\)' | head -1)
money_alert=""
